...
function fuse_PEPO_PEPS(O, A, fuser)
    isdual(PEPSKit.virtualspace(O, 1)) || twist!(O, 3)
    isdual(PEPSKit.virtualspace(O, 2)) || twist!(O, 4)

    isdual(PEPSKit.virtualspace(A, 1)) || twist!(A, 2)
    isdual(PEPSKit.virtualspace(A, 2)) || twist!(A, 3)
...
projector_alg = projector_algs[1]
@testset "mpotensor for PEPOTraceSandwich using $alg with $projector_alg" for alg in ctm_styles
    ...
    env, = leading_boundary(CTMRGEnv(mpo, envspace), mpo; alg, maxiter=1000, projector_alg)
    env_fused, = leading_boundary(
        CTMRGEnv(mpo_fused, envspace), mpo_fused; alg, maxiter=500, projector_alg
    )
    println(abs(network_value(mpo, env) - network_value(mpo_fused, env_fused)))
    @test network_value(mpo, env) ≈ network_value(mpo_fused, env_fused) atol = 1e-4
end
...
@testset "PEPO layers CTMRG contraction using $alg with $projector_alg" for (
    ...
    env, = leading_boundary(
        CTMRGEnv(network, envspace), network; alg, maxiter=750, projector_alg
    )
    env_fused, = leading_boundary(
        CTMRGEnv(network_fused, envspace), network_fused; alg, maxiter=750, projector_alg
    )
    ...
    println(abs((m / nrm) - (m_fused / nrm_fused)))
    @test (m / nrm) ≈ (m_fused / nrm_fused) atol = 5e-3
end
...
@testset "Test adjoint of an InfinitePEPO using $alg with $projector_alg" for alg in
                                                                              ctm_styles
    # Test the definition of the adjoint of an operator, i.e. dot(phi, O, psi) == dot(phi, O * psi) == dot(O' * phi, psi) for any two states psi and phi.
    Oψ = fuse_PEPO_PEPS(O, ψ, fuser)
    Odagϕ = fuse_PEPO_PEPS(PEPSKit._dag(O), ϕ, fuser_adj)

    network_ψOϕ = InfiniteSquareNetwork(
        InfinitePEPS(ψ), InfinitePEPO(O), InfinitePEPS(ϕ)
    )
    ...
    overlap2 = network_value(network_Oψ_ϕ, env_Oψ_ϕ)
    overlap3 = network_value(network_ψ_Odagϕ, env_ψ_Odagϕ)
